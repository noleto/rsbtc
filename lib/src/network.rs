...
impl Message {
    ...
    pub async fn send_async<W>(&self, stream: &mut W) -> Result<(), ciborium::ser::Error<IoError>>
    where
        W: AsyncWrite + Unpin,
    {
        let bytes = self.encode()?;
    ...
    pub async fn receive_async<R>(stream: &mut R) -> Result<Self, ciborium::de::Error<IoError>>
    where
        R: AsyncRead + Unpin,
    {
